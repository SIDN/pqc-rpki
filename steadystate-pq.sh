# This script demonstrates the steady state using post-quantum keys.
# We create a tree of TA > online CA > child CA > grandchild CA,
# with two separate Krill instances, all using post-quantum keys.
#
# We create the following snapshots:
# 1:  Using only post-quantum keys.
#     - "123.12.23.0/24-24 => 5" by the child.
#     - "123.12.34.0/24-24 => 5" by the grandchild.

set -e

export LOCATION="data/steadystate-pq" \
       KRILL_CLI_TOKEN="foo" \
       TA_CONF="configs/krillta-pq-traditional.conf"
...
krill -c configs/krill-pq-traditional.conf &
ONLINE_PID=$!
...
# Create a ROA in the child krill instance, using the post-quantum keypair.
...
# Create a ROA in the grandchild krill instance, using a post-quantum keypair.
